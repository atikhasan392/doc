...
Run the following script to generate a live version summary of every installed tool:
...
print_row() {
  ...
  local cmd="$2"
  local version
  version=$(eval "$cmd" 2>/dev/null | head -n1 | grep -oP '[\d]+\.[\d]+\.?[\d]*' | head -n1)
  if [ -n "$version" ]; then
    printf "| %-14s | %-30s | %-6s |\n" "$tool" "$version" "✅"
  else
    printf "| %-14s | %-30s | %-6s |\n" "$tool" "Not found" "❌"
...
echo ""
echo "+----------------+--------------------------------+--------+"
echo "| Tool           | Installed Version              | Status |"
echo "+----------------+--------------------------------+--------+"
print_row "PHP"          "php --version"
print_row "PHP-FPM"      "php-fpm8.5 --version"
print_row "Composer"     "composer --version"
print_row "Laravel CLI"  "laravel --version"
print_row "MySQL"        "mysql --version"
print_row "Redis"        "redis-cli --version"
print_row "Node.js"      "node --version"
print_row "NPM"          "npm --version"
print_row "Yarn"         "yarn --version"
print_row "Bun"          "bun --version"
print_row "Apache"       "apache2 -v"
print_row "GitHub CLI"   "gh --version"
echo "+----------------+--------------------------------+--------+"
echo ""
```
...
```bash
php -m | grep -E "bcmath|curl|dom|gd|mbstring|pdo_mysql|pdo_sqlite|xml|zip|redis|intl" \
  | sort \
  | awk '{print "  ✅ " $0}'
```

### PHP Settings Verification

```bash
php -i | grep -E \
  "memory_limit|upload_max_filesize|post_max_size|max_input_vars|date.timezone|display_errors|max_file_uploads|realpath_cache_size"
```
